...
> 最后更新:2026-09-15。本文是**设计决策的权威存档**,代码改动前先读这里。

## 一、分层结构

```
┌─────────────────────────────────────────────┐
│  net/        myMuduo(已写,直接接入)          │  收发字节流
├─────────────────────────────────────────────┤
│  protocol/   RESP 解析器                      │  字节流 → 命令
├─────────────────────────────────────────────┤
│  command/    命令层                           │  SET/GET/HGET… → 调存储
├─────────────────────────────────────────────┤
│  storage/    InMemoryDB                       │  按 key 查对象
├─────────────────────────────────────────────┤
│  object/     RedisObject + 类型体系            │  类型从这里开始存在
├─────────────────────────────────────────────┤
│  core/       sds / dict / skiplist            │  通用原语,零 Redis 知识
└─────────────────────────────────────────────┘
```
...
| 层 | 内容 | 状态 |
|---|---|---|
| core | `SDS`(动态字符串)、`DICT<V>`(哈希表) | ✅ 完成 |
| object | `redisObject`(类型体系 + RAII 所有权) | ✅ 完成 |
| storage | `InMemoryDB` | ⬜ 待写 |
| protocol | RESP2 增量解析器 | ⬜ 待写 |
| command | 命令实现 | ⬜ 待写 |
| net | myMuduo 接入 | ⬜ 待写 |
| persistence | AOF | ⬜ 待写(目标 v1) |
...
class redisObject final {
    enum class RedisObjectType     { STRING, HASH, /* ZSet… */ };
    enum class RedisObjectEncoding { RAW, HASHTABLE, /* … */ };
    RedisObjectType     type_;
    RedisObjectEncoding encoding_;
    void*               ptr_;      // 指向载荷(SDS* / DICT<SDS>*)
};
```

- **具名工厂 + 私有构造**:`createStringObject` / `createHashObject` 返回 `unique_ptr`
...
- 收益具体化:ZSet 内层 `DICT<SDS>` 可直接 `dict.set(member, score)`,零 cast、零手动释放
...
- **ZSet ≠ 跳表**:双索引 = 跳表(有序/范围/rank)+ `DICT<SDS>`(member→score 精确查, O(1))。只放跳表会让 ZSCORE/判重变 O(n)
- **跳表放 core**(通用有序容器),Redis 语义包装在 object 层的 ZSet 类
- **编码体系**:v1 只落地 String-RAW 和 Hash-HASHTABLE;int / embstr 设计预留。embstr 需对象头与 SDS 共享分配,为不污染 core 层 SDS 通用性而不做
- **AOF 纳入 v1**:回放 / 重写 / 三种 fsync 策略
