...
用 C++17 从零实现的单线程 Redis：自写的 RESP2 协议解析器、跳表有序集合、AOF 持久化，
网络层基于自写的 myMuduo（epoll 事件循环）。**不依赖 hiredis、redis-plus-plus、boost**。

## 特性

- **协议**：手写 RESP2 增量解析器，支持简单字符串、错误、整数、批量字符串和数组；半包自动等待、粘包连续解析
- **数据结构**：String、Hash、ZSet（跳表与字典双索引，支持 rank 与范围查询）
- **37 条命令**，大小写不敏感，覆盖 Connection、String、Key、Hash、ZSet 五类
- **过期键**：惰性删除与定时采样主动淘汰（类似 Redis 的 `activeExpireCycle`）
- **持久化**：AOF，支持 `always`、`everysec`、`no` 三种刷盘策略；前台重写与**后台重写（`bgrewriteaof`）**，重写期间新写入按缓冲增量合并
...
command/     命令分发与命令实现           SET、GET、HGET 等 → 调存储
storage/     InMemoryDB                   按 key 查对象与过期管理
object/      RedisObject 与 ZSet          类型从这里开始存在
core/        SDS、DICT<V>、SkipList       通用原语，零 Redis 知识
persistence/ AOF                          命令追加、重写与回放
```
...
macOS 与 Windows 上仍可编译 `miniredis_core` 与单元测试。
...
8 个测试二进制、147 个用例，按层分开：sds、dict、skiplist、object、zset、storage、resp、command。
首次配置会通过 FetchContent 拉取 googletest，需要能访问 GitHub。

## 压测

压测工具为 `redis-benchmark`（Redis 6.0.16），在 3 核容器内进行。服务器以**单线程**运行，
并开启 AOF（`appendfsync everysec`），构建参数为 `-O3 -DNDEBUG`；每个测点重启服务、
预置 10 万键，重复 3 次取中位。复现脚本见 [scripts/bench.sh](scripts/bench.sh)。
...
- `mset` 一条请求写 **10 对键值**（redis-benchmark 的固定模板），单位工作量是 `set` 的 10 倍，所以 QPS 低不代表实现慢。
- `zpopmin` 压的是**空 zset**：模板是 `ZPOPMIN myzset`（不带 `__rand_int__`，且只弹不压），数字反映的是空转路径，不能当作跳表删除性能看。
- `zadd` 走跳表插入，比哈希表写入略慢。
...
单连接约 3.6 万 QPS 由往返延迟主导；并发 10 就已把单线程打满，之后基本持平。
...
- 只实现 RESP2，且**不支持内联命令**（inline protocol）：收到非 `*` 开头的输入会回协议错误并关闭连接，所以 `redis-benchmark` 的 `PING_INLINE` 用不了
- 未实现 `CONFIG`，客户端连接后的例行探测会拿到 `ERR unknown command 'CONFIG'`
- 数据类型只有 String、Hash、ZSet，没有 List、Set
- `SET` 不支持 `EX` 等选项；`EXPIRE` 写进 AOF 的是相对秒数，回放时按回放时刻重新计时，与真 Redis 存绝对时间戳的做法不同
- 单线程，无主从、集群、哨兵
- 持久化只有 AOF，没有 RDB 快照
